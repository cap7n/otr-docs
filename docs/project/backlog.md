...
- [ ] 🔨 **Turret jitters on the host while the car moves — fix applied 2026-07-05, pending re-test:** diagnosis confirmed in-session (parked = clean, rolling = jitter); the two raw `pitch_pivot.global_transform` reads in `_update_barrels` / `_get_convergence_point` now use `_get_pivot_transform()`. Original diagnosis: observed 2026-07-05 (host view, any seat, either driver; client view fine). Cause: on the server, turret body mesh + camera are posed from the INTERPOLATED pivot via `_get_pivot_transform()`, but `_update_barrels()` (turret.gd:182) and `_get_convergence_point()` (turret.gd:234) read raw `pitch_pivot.global_transform` — the un-interpolated physics-tick pose — so the barrels are aimed one time base apart from the body they're mounted on and micro-flick whenever the car moves. Client is coherent because it uses raw everywhere. The fix helper already exists in the same file (the muzzle-point code at turret.gd:213 documents the exact lesson); swap the two raw reads to `_get_pivot_transform()`. Confirm first: parked = no jitter, rolling = jitter. Not the computing side — turret is host-simulated (gunner only sends input deltas); the aim RPC being per-mouse-event unreliable is a separate, minor smell
