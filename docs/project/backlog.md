...
- [ ] **Turret jitters (host view, client driving):** observed 2026-07-05 during the post-refactor multiplayer test — everything else smooth, only the turret. Hypothesis A: the GUN camera path bypasses the interpolated-transform machinery every other seat uses (player.gd anchors straight to `turret.camera_anchor` and camera_controller copies raw `global_transform` from a physics-stepped RigidBody3D parent — the exact jitter recipe `_get_xform()` exists to prevent). Hypothesis B: turret aim state smoothed/synced on a different clock than the car it's parented to. Note: turret.gd (391 lines) is the one Vehicles file never audited. Cheap diagnostics before fixing: does it mirror (host driving, client gunning)? does it persist with the car parked?
