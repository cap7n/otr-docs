...
- [ ] **Zone footprint rework:** 2x2 km of pure yard reads too big. Direction: fenced yard core (~600-800 m, dense) + the extracted surroundings (access road, wasteland, shelter outside the fence) - fits the sampling fiction and gives a sparse->dense intensity gradient. Reshaping in progress
...
### String references (2026-07-15 sweep — full log with every call site: [String References Audit](string-references.md))

All 115 scripts swept for strings that name nodes, methods, groups, anims, or resources - each one breaks silently on rename because the editor never rewrites `.gd` files. Counts and line numbers live on the audit page; these are the actionable slices:

- [ ] **Kill node-path-as-network-identity (audit §1, the one HIGH item):** NTM's sync registry is keyed by `str(entity.get_path())`, and turret impulse, inventory drag, and item pickup RPCs all ship `get_path()` strings that the receiver re-resolves with `get_node_or_null` - tree-order divergence or a dropped packet means silent no-op or the *wrong node*. Migrate the streams to NetId ints (autoload already exists and stamps `net_id` meta). Same disease as the phantom-rocket item above - do them in the same session, one stream at a time, host+client test after each
- [ ] **Group-name constants (cheapest on the list, ~1 h):** 50 call sites hand-type 11 group strings (`enemy`, `targetable`, `interactable`, …); one typo'd `add_to_group` = an enemy the flamethrower can't see, nothing logs. Add a `Groups` block of StringName consts in Globals, mechanical find-replace
- [ ] **`Damageable` base class instead of `has_method("take_damage")`:** the damage interface is string-matched at 11 sites (+2 for `apply_explosion_force`); a base class + `is` check turns silent no-ops into compile errors. Same pattern then available for the enemy-pool and sync interfaces (audit §3)
- [ ] **De-string the turret + car-satellite node paths:** turret.gd reaches through 14 deep `$Base/PitchPivot/Boddy/…` chains (the `Boddy` typo is load-bearing in 8 of them - fix name and paths together, editor rename first so the .tscn follows), and seat/dock/audio/item-behavior scripts grab car children by name string. Convert to `@export NodePath` (visual_damage_controller already shows the pattern) or `%` unique names. Also: engine_sound_blender builds node names *dynamically* (`"OnLoop%d" % rpm`) - grep-proof, document or restructure
- [ ] **WorldManager carry-over reads gameplay state by string:** `car_node.get("drone_lives")`, `player_node.get("_current_seat")`, `get("stasis_zone")` (WorldManager.gd:167-183) - rename any of these and run carry-over quietly stops carrying. Give them real getters (audit §5; debug_hud's ~15 string reads are debug-only, left as-is)
- [ ] **preload the 6 runtime `load("res://…")` sites:** moved file = runtime error only when that code path fires; preload fails at parse instead. game_over_menu.gd:17 especially - a hardcoded level path in a UI script, duplicating WorldManager's job
- [ ] **Anim-name constants + startup validation in character_animator:** 9 anim strings; add one setup loop that `push_warning`s every clip missing from the AnimationPlayer so a bad re-import reports at spawn, not one state at a time in play - would have caught `kneel_idle` (see 4.7 aftercare)
