...
- [ ] **Phantom .blend rewrites (unsolved, parked 2026-07-05):** Car.blend/Turret.blend byte-changed in commits on BOTH machines with nobody opening Blender for days — always clustered around Godot editor sessions. Not a documented Godot behavior (searched). Harmless so far (re-saves are content-benign), but unexplained. To catch it: watch the files' write-times while forcing a Reimport of Car.blend and check if headless blender.exe is alive at that second. Until then: glance at `git diff --stat` before committing so phantom blend edits don't ride along
